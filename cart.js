  ...
  for (let i = 0; i < document.querySelectorAll(".delete").length; i++) {
    document
      ...
      [i].addEventListener("click", function () {
        fetch(`https://trainlife-back.manupuyuelo.com/cart/${this.id}`, {
          method: "DELETE",
        })
...
// Get cart
fetch("https://trainlife-back.manupuyuelo.com/cart")
  .then((response) => response.json())
  .then((data) => {
    if (data.result) {
...
document.querySelector("#purchase").addEventListener("click", function () {
  fetch("https://trainlife-back.manupuyuelo.com/bookings", { method: "PUT" })
    .then((response) => response.json())
    .then((data) => {
